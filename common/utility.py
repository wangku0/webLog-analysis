...
class ImageCode:
    ...
    def draw_verify_code(self):
        ...
        return im, code  #返回图片和值
    # 生成图片验证码并返回给控制器
    def get_code(self):
        #生成图片和值
        image, code = self.draw_verify_code()
        buf = BytesIO() #二进制对象存储数据
        image.save(buf, 'jpeg') #保存到内存  即缓冲区
        bstring = buf.getvalue() #获取二进制编码
        return code, bstring  #将图片的值和二进制数据返回给控制器
...
def send_email(receiver, ecode):#receiver 收件人    ecode邮箱验证码
    sender = 'WoniuNote <2676011245@qq.com>'  # 你的邮箱账号和发件者签名
    ...
    smtpObj.login(user='2676011245@qq.com', password='fafqeehlqfandjhf')
    # 指定发件人，收件人和邮件内容   发送邮件核心操作
